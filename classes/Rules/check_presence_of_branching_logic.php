...
class check_presence_of_branching_logic implements ValidationsImplementation
{
    ...
    public function validate(): bool
    {
        ...
        //\REDCap::email('msherm12@jh.edu', 'redcap@jh.edu', 'branching_fields', json_encode($branching_fields));
        ...
        $FailedComparisons = self::findNonMatchingComparisonRows(
            $comparisonrows,
            $RadioDropDownFields
        );
...
// Split on the first occurrence of the value/label delimiter only
...
// Normalize: strip trailing :value and trim
    ...
    function buildRadioDropdownIndex(array $radioDropdownFields): array
    {
        ...
        foreach ($radioDropdownFields as $r) {
            ...
            $idx[$field] = $idx[$field] ?? [];
            foreach ($vals as $v) { $idx[$field][(string)$v] = true; }
        }
    ...
    function findNonMatchingComparisonRows(
        ...
        foreach ($comparisonRows as $row) {
            ...
            if (!isset($index[$field])) {
                if ($includeUnknownFields) { $out[] = [$field, $value]; }
            ...
            if ($looseNumericMatch && is_numeric($value)) {
                foreach ($allowed as $code => $_) {
                    if (is_numeric($code) && (string) (+$code) === (string) (+$value)) {
                        $matched = true; break; // numeric-equal; treat as match
                    }
                }
            }


            if (!$matched) { $out[] = array($row[0],$row[1],$field, $value); }
        }
